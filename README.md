...
| **Direct TUI actions + granular load balancer controls** | Quota/verify/configure required leaving the account manager, and load-balancer tuning was manual | `Check quotas`, `Verify all`, `Verify this account`, and `Configure models` now run directly in `/ag-accounts`; plus one-click cache-friendly defaults (`hybrid + cache_first`) and a full `Load balancer settings` dialog (strategy, scheduling, PID offset, retry/backoff/jitter, quota refresh tuning) |
